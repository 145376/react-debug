 ...
 * 
 */
import { current, runWithFiberInDEV } from 'react-reconciler/src/ReactCurrentFiber';
import { HostComponent, HostHoistable, HostSingleton, HostText } from 'react-reconciler/src/ReactWorkTags';
import { describeDiff } from 'react-reconciler/src/ReactFiberHydrationDiffs';

function describeAncestors(ancestor, child, props) {
  var fiber = child;
  var node = null;
  var distanceFromLeaf = 0;
...
var buttonScopeTags = __DEV__ ? inScopeTags.concat(['button']) : []; // https://html.spec.whatwg.org/multipage/syntax.html#generate-implied-end-tags
...
function updatedAncestorInfoDev(oldInfo, tag) {
  if (__DEV__) {
    var ancestorInfo = Object.assign({}, oldInfo || emptyAncestorInfoDev);
    var info = {
      tag: tag
    };
    ...
    if (oldInfo === null && (tag === '#document' || tag === 'html' || tag === 'body')) {
...
function validateDOMNesting(childTag, ancestorInfo) {
  if (__DEV__) {
    ...
    var invalidParent = isTagValidWithParent(childTag, parentTag, ancestorInfo.implicitRootScope) ? null : parentInfo;
    ...
    var child = current;
    var ancestor = child ? findAncestor(child.return, ancestorTag) : null;
    var ancestorDescription = child !== null && ancestor !== null ? describeAncestors(ancestor, child, null) : '';
    ...
    if (child) {
      ...
      if (ancestor !== null && parent !== null && (ancestor !== parent || parent._debugOwner !== child._debugOwner)) {
        runWithFiberInDEV(ancestor, function () {
          console.error( // We repeat some context because this log might be taken out of context
...
function validateTextNesting(childText, parentTag, implicitRootScope) {
  if (__DEV__) {
    ...
    didWarn[warnKey] = true;
    var child = current;
    var ancestor = child ? findAncestor(child, parentTag) : null;
    var ancestorDescription = child !== null && ancestor !== null ? describeAncestors(ancestor, child, child.tag !== HostText ? {
      children: null
    } : null) : '';
