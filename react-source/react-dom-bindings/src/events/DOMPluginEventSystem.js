...
import { HostRoot, HostPortal, HostComponent, HostHoistable, HostSingleton, HostText, ScopeComponent } from 'react-reconciler/src/ReactWorkTags';
...
import { enableLegacyFBSupport, enableCreateEventHandleAPI, enableScopeAPI, disableCommentsAsDOMContainers, enableScrollEndPolyfill } from 'shared/ReactFeatureFlags';
...
function extractEvents(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  if (enableScrollEndPolyfill) {
    ScrollEndEventPlugin.extractEvents(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer);
  }
...
export var nonDelegatedEvents = new Set(['beforetoggle', 'cancel', 'close', 'invalid', 'load', 'scroll', 'scrollend', 'toggle'].concat(mediaEventTypes));
  ...
  if (inCapturePhase) {
    ...
    for (var _i = 0; _i < dispatchListeners.length; _i++) {
      ...
      if (__DEV__ && _instance !== null) {
        runWithFiberInDEV(_instance, executeDispatch, event, _listener, _currentTarget);
      } else {
...
export function accumulateEnterLeaveTwoPhaseListeners(dispatchQueue, leaveEvent, enterEvent, from, to) {
  var common = from && to ? getLowestCommonAncestor(from, to, getParent) : null;
