...
import { processDispatchQueue, accumulateSinglePhaseListeners, accumulateTwoPhaseListeners } from '../DOMPluginEventSystem';
import { getScrollEndTimer, setScrollEndTimer, clearScrollEndTimer } from '../../client/ReactDOMComponentTree';
import { enableScrollEndPolyfill } from 'shared/ReactFeatureFlags';
var isScrollEndEventSupported = enableScrollEndPolyfill && canUseDOM && isEventSupported('scrollend');
var isTouchStarted = false;
var isMouseDown = false;

function registerEvents() {
  registerTwoPhaseEvent('onScrollEnd', ['scroll', 'scrollend', 'touchstart', 'touchcancel', 'touchend', 'mousedown', 'mouseup']);
}

function manualDispatchScrollEndEvent(inst, nativeEvent, target) {
  var dispatchQueue = [];
  var listeners = accumulateTwoPhaseListeners(inst, 'onScrollEnd');

  if (listeners.length > 0) {
    var event = new SyntheticUIEvent('onScrollEnd', 'scrollend', null, nativeEvent, // This will be the "scroll" event.
    target);
    dispatchQueue.push({
      event: event,
      listeners: listeners
...
function debounceScrollEnd(targetInst, nativeEvent, nativeEventTarget) {
  var existingTimer = getScrollEndTimer(nativeEventTarget);
  ...
  if (targetInst !== null) {
    var newTimer = setTimeout(fireScrollEnd.bind(null, targetInst, nativeEvent, nativeEventTarget), DEBOUNCE_TIMEOUT);
...
function extractEvents(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  var listeners = accumulateSinglePhaseListeners(targetInst, 'onScrollEnd', 'scrollend', inCapturePhase, accumulateTargetOnly, nativeEvent);

  if (listeners.length > 0) {
    // Intentionally create event lazily.
    var event = new SyntheticUIEvent('onScrollEnd', 'scrollend', null, nativeEvent, nativeEventTarget);
    dispatchQueue.push({
      event: event,
      listeners: listeners
    });
