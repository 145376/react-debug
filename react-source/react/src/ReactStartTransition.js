...
export function startTransition(scope, options) {
  ...
  if (enableViewTransition) {
    currentTransition.types = prevTransition !== null ? // If we're a nested transition, we should use the same set as the parent
  ...
  if (enableTransitionTracing) {
    currentTransition.name = options !== undefined && options.name !== undefined ? options.name : null;
...
export function startGestureTransition(provider, scope, options) {
  if (!enableGestureTransition) {
    // eslint-disable-next-line react-internal/prod-error-codes
    throw new Error('startGestureTransition should not be exported when the enableGestureTransition flag is off.');
  }
