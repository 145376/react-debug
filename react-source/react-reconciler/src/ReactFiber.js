 ...
 * 
 */
import { supportsResources, supportsSingletons, isHostHoistableType, isHostSingletonType } from './ReactFiberConfig';
import { enableProfilerTimer, enableScopeAPI, enableLegacyHidden, enableTransitionTracing, enableDO_NOT_USE_disableStrictPassiveEffect, enableRenderableContext, disableLegacyMode, enableObjectFiber, enableViewTransition, enableSuspenseyImages } from 'shared/ReactFeatureFlags';
import { NoFlags, Placement, StaticMask } from './ReactFiberFlags';
import { ConcurrentRoot } from './ReactRootTags';
import { ClassComponent, HostRoot, HostComponent, HostText, HostPortal, HostHoistable, HostSingleton, ForwardRef, Fragment, Mode, ContextProvider, ContextConsumer, Profiler, SuspenseComponent, SuspenseListComponent, DehydratedFragment, FunctionComponent, MemoComponent, SimpleMemoComponent, LazyComponent, ScopeComponent, OffscreenComponent, LegacyHiddenComponent, TracingMarkerComponent, Throw, ViewTransitionComponent, ActivityComponent } from './ReactWorkTags';
...
import { NoMode, ConcurrentMode, ProfileMode, StrictLegacyMode, StrictEffectsMode, NoStrictPassiveEffectsMode, SuspenseyImagesMode } from './ReactTypeOfMode';
import { REACT_FORWARD_REF_TYPE, REACT_FRAGMENT_TYPE, REACT_STRICT_MODE_TYPE, REACT_PROFILER_TYPE, REACT_PROVIDER_TYPE, REACT_CONTEXT_TYPE, REACT_CONSUMER_TYPE, REACT_SUSPENSE_TYPE, REACT_SUSPENSE_LIST_TYPE, REACT_MEMO_TYPE, REACT_LAZY_TYPE, REACT_SCOPE_TYPE, REACT_LEGACY_HIDDEN_TYPE, REACT_TRACING_MARKER_TYPE, REACT_ELEMENT_TYPE, REACT_VIEW_TRANSITION_TYPE, REACT_ACTIVITY_TYPE } from 'shared/ReactSymbols';
...
//   try {
//     var nonExtensibleObject = Object.preventExtensions({}); // eslint-disable-next-line no-new

//     new Map([[nonExtensibleObject, null]]); // eslint-disable-next-line no-new
...
export function isFunctionClassComponent(type) {
...
export function createWorkInProgress(current, pendingProps) {
  ...
  workInProgress.dependencies = currentDependencies === null ? null : __DEV__ ? {
...
export function createFiberFromActivity(pendingProps, mode, lanes, key) {
  var fiber = createFiber(ActivityComponent, pendingProps, key, mode);
  fiber.elementType = REACT_ACTIVITY_TYPE;
  fiber.lanes = lanes;
  return fiber;
}
export function createFiberFromViewTransition(pendingProps, mode, lanes, key) {
  ...
  var fiber = createFiber(ViewTransitionComponent, pendingProps, key, mode);
...
export function createFiberFromThrow(error, mode, lanes) {
  var fiber = createFiber(Throw, error, null, mode);
  fiber.lanes = lanes;
  return fiber;
}