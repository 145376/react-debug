 ...
 * 
 */
import { disableLegacyMode, disableSchedulerTimeoutInWorkLoop, enableProfilerTimer, enableProfilerNestedUpdatePhase, enableComponentPerformanceTrack, enableYieldingBeforePassive, enableGestureTransition, enableDefaultTransitionIndicator } from 'shared/ReactFeatureFlags';
import { NoLane, NoLanes, SyncLane, DefaultLane, getHighestPriorityLane, getNextLanes, includesSyncLane, markStarvedLanesAsExpired, claimNextTransitionLane, getNextLanesToFlushSync, checkIfRootIsPrerendering, isGestureRender } from './ReactFiberLane';
import { CommitContext, NoContext, RenderContext, flushPendingEffects, getExecutionContext, getWorkInProgressRoot, getWorkInProgressRootRenderLanes, getRootWithPendingPassiveEffects, getPendingPassiveEffectsLanes, hasPendingCommitEffects, isWorkLoopSuspendedOnData, performWorkOnRoot } from './ReactFiberWorkLoop';
import { LegacyRoot } from './ReactRootTags';
import { ImmediatePriority as ImmediateSchedulerPriority, UserBlockingPriority as UserBlockingSchedulerPriority, NormalPriority as NormalSchedulerPriority, IdlePriority as IdleSchedulerPriority, cancelCallback as Scheduler_cancelCallback, scheduleCallback as Scheduler_scheduleCallback, now } from './Scheduler';
import { DiscreteEventPriority, ContinuousEventPriority, DefaultEventPriority, IdleEventPriority, lanesToEventPriority } from './ReactEventPriorities';
import { supportsMicrotasks, scheduleMicrotask, shouldAttemptEagerTransition, trackSchedulerEvent, noTimeout } from './ReactFiberConfig';
import ReactSharedInternals from 'shared/ReactSharedInternals';
import { resetNestedUpdateFlag, syncNestedUpdateFlag } from './ReactProfilerTimer';
import { peekEntangledActionLane } from './ReactFiberAsyncAction';
import noop from 'shared/noop';
import reportGlobalError from 'shared/reportGlobalError';
import { startIsomorphicDefaultIndicatorIfNeeded, hasOngoingIsomorphicIndicator, retainIsomorphicIndicator, markIsomorphicIndicatorHandled } from './ReactFiberAsyncAction'; // A linked list of all the roots with pending work. In an idiomatic app,
// there's only a single root, but we do support multi root apps, hence this
// extra complexity. But this module is optimized for the single root case.

export var firstScheduledRoot = null;
...
export function ensureRootIsScheduled(root) {
  ...
  if (root === lastScheduledRoot || root.next !== null) {// Fast path. This root is already scheduled.
  } else {
  ...
  if (__DEV__ && !disableLegacyMode && ReactSharedInternals.isBatchingLegacy && root.tag === LegacyRoot) {
...
function flushSyncWorkAcrossRoots_impl(syncTransitionLanes, onlyLegacy) {
  ...
  do {
    ...
    while (root !== null) {
      if (onlyLegacy && (disableLegacyMode || root.tag !== LegacyRoot)) {// Skip non-legacy roots.
      } else {
        if (syncTransitionLanes !== NoLanes) {
          var nextLanes = getNextLanesToFlushSync(root, syncTransitionLanes);
          ...
          var rootHasPendingCommit = root.cancelPendingCommit !== null || root.timeoutHandle !== noTimeout;

          var _nextLanes = getNextLanes(root, root === workInProgressRoot ? workInProgressRootRenderLanes : NoLanes, rootHasPendingCommit);

          if ((includesSyncLane(_nextLanes) || enableGestureTransition && isGestureRender(_nextLanes)) && !checkIfRootIsPrerendering(root, _nextLanes)) {
            // This root has pending sync work. Flush it now.
            didPerformSomeWork = true;
            performSyncWorkOnRoot(root, _nextLanes);
          }
        }
      }
...
function processRootScheduleInMicrotask() {
  ...
  var syncTransitionLanes = NoLanes;
  ...
  while (root !== null) {
    ...
    if (nextLanes === NoLane) {
      ...
      prev = root; // This is a fast-path optimization to early exit from
      ...
      if ( // Skip the optimization if syncTransitionLanes is set
...
function startDefaultTransitionIndicatorIfNeeded() {
  ...
  startIsomorphicDefaultIndicatorIfNeeded(); // Check all the roots if there are any new indicators needed.
  ...
  while (root !== null) {
    if (root.indicatorLanes !== NoLanes && root.pendingIndicator === null) {
      ...
      if (hasOngoingIsomorphicIndicator()) {
        ...
        try {
          var onDefaultTransitionIndicator = root.onDefaultTransitionIndicator;
...
function scheduleTaskForRootDuringMicrotask(root, currentTime) {
  ...
  var rootWithPendingPassiveEffects = getRootWithPendingPassiveEffects();
  var pendingPassiveEffectsLanes = getPendingPassiveEffectsLanes();
  var workInProgressRoot = getWorkInProgressRoot();
  var workInProgressRootRenderLanes = getWorkInProgressRootRenderLanes();
  var rootHasPendingCommit = root.cancelPendingCommit !== null || root.timeoutHandle !== noTimeout;
  var nextLanes = enableYieldingBeforePassive && root === rootWithPendingPassiveEffects ? // This will schedule the callback at the priority of the lane but we used to
  ...
  pendingPassiveEffectsLanes : getNextLanes(root, root === workInProgressRoot ? workInProgressRootRenderLanes : NoLanes, rootHasPendingCommit);
  ...
  !checkIfRootIsPrerendering(root, nextLanes)) {
    ...
    // on the `act` queue.
    !(__DEV__ && ReactSharedInternals.actQueue !== null && existingCallbackNode !== fakeActCallbackNode)) {
    ...
    var newCallbackNode = scheduleCallback(schedulerPriorityLevel, performWorkOnRootViaSchedulerTask.bind(null, root));
...
function performWorkOnRootViaSchedulerTask(root, didTimeout) {
  ...
  var didFlushPassiveEffects = flushPendingEffects(true);
  ...
  var workInProgressRootRenderLanes = getWorkInProgressRootRenderLanes();
  var rootHasPendingCommit = root.cancelPendingCommit !== null || root.timeoutHandle !== noTimeout;
  var lanes = getNextLanes(root, root === workInProgressRoot ? workInProgressRootRenderLanes : NoLanes, rootHasPendingCommit);
  ...
  var forceSync = !disableSchedulerTimeoutInWorkLoop && didTimeout;
  performWorkOnRoot(root, lanes, forceSync); // The work loop yielded, but there may or may not be work left at the current
...
function performSyncWorkOnRoot(root, lanes) {
  ...
  // through Scheduler.
  var didFlushPassiveEffects = flushPendingEffects();
...
export function requestTransitionLane( // This argument isn't used, it's only here to encourage the caller to
...
transition) {
  ...
  if (currentEventTransitionLane === NoLane) {
    ...
    currentEventTransitionLane = actionScopeLane !== NoLane ? // We're inside an async action scope. Reuse the same lane.
