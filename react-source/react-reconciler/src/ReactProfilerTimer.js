 ...
 * 
 */
import { isTransitionLane, isBlockingLane, isSyncLane, includesTransitionLane, includesBlockingLane, includesSyncLane } from './ReactFiberLane';
import { resolveEventType, resolveEventTimeStamp } from './ReactFiberConfig';
import { enableProfilerCommitHooks, enableProfilerNestedUpdatePhase, enableProfilerTimer, enableComponentPerformanceTrack } from 'shared/ReactFeatureFlags';
import { isAlreadyRendering } from './ReactFiberWorkLoop'; // Intentionally not named imports because Rollup would use dynamic dispatch for
// CommonJS interop named imports.

import * as Scheduler from 'scheduler';
var now = Scheduler.unstable_now;
var createTask = // eslint-disable-next-line react-internal/no-production-logging
__DEV__ && console.createTask ? // eslint-disable-next-line react-internal/no-production-logging
console.createTask : function (name) {
  return null;
};
export var renderStartTime = -0;
export var commitStartTime = -0;
export var commitEndTime = -0;
export var commitErrors = null;
export var profilerStartTime = -1.1;
export var profilerEffectDuration = -0;
export var componentEffectDuration = -0;
export var componentEffectStartTime = -1.1;
export var componentEffectEndTime = -1.1;
export var componentEffectErrors = null;
export var blockingClampTime = -0;
export var blockingUpdateTime = -1.1; // First sync setState scheduled.

export var blockingUpdateTask = null; // First sync setState's stack trace.

export var blockingEventTime = -1.1; // Event timeStamp of the first setState.

export var blockingEventType = null; // Event type of the first setState.

export var blockingEventIsRepeat = false;
export var blockingSpawnedUpdate = false;
export var blockingSuspendedTime = -1.1; // TODO: This should really be one per Transition lane.

export var transitionClampTime = -0;
export var transitionStartTime = -1.1; // First startTransition call before setState.

export var transitionUpdateTime = -1.1; // First transition setState scheduled.

export var transitionUpdateTask = null; // First transition setState's stack trace.

export var transitionEventTime = -1.1; // Event timeStamp of the first transition.

export var transitionEventType = null; // Event type of the first transition.

export var transitionEventIsRepeat = false;
export var transitionSuspendedTime = -1.1;
export var yieldReason = 0;
export var yieldStartTime = -1.1; // The time when we yielded to the event loop
  ...
  if (isSyncLane(lane) || isBlockingLane(lane)) {
    ...
    if (transitionUpdateTime < 0) {
      ...
      if (transitionStartTime < 0) {
        var _newEventTime = resolveEventTimeStamp();

        var _newEventType = resolveEventType();

        if (_newEventTime !== transitionEventTime || _newEventType !== transitionEventType) {
          transitionEventIsRepeat = false;
        }

        transitionEventTime = _newEventTime;
        transitionEventType = _newEventType;
      }
    }
  }
}
...
export function bubbleNestedEffectDurations(prevEffectDuration) {
  if (!enableProfilerTimer || !enableProfilerCommitHooks) {
    return 0;
  }
...
export function popComponentEffectErrors(prevErrors) {
  if (!enableProfilerTimer || !enableProfilerCommitHooks) {
    return;
  }
...
export function stopProfilerTimerIfRunningAndRecordDuration(fiber) {
  if (!enableProfilerTimer) {
    return;
  }
...
export function stopProfilerTimerIfRunningAndRecordIncompleteDuration(fiber) {
  if (!enableProfilerTimer) {
    return;
  }
  ...
  if (profilerStartTime >= 0) {
    var elapsedTime = now() - profilerStartTime;
    fiber.actualDuration += elapsedTime; // We don't update the selfBaseDuration here because we errored.
...
export function recordEffectDuration(fiber) {
  ...
  if (profilerStartTime >= 0) {
    ...
    profilerStartTime = -1; // Store duration on the next nearest Profiler ancestor
    // Or the root (for the DevTools Profiler to read)

    profilerEffectDuration += elapsedTime;
    componentEffectDuration += elapsedTime; // Keep track of the last end time of the effects.
