...
import { enableComponentPerformanceTrack, enableProfilerTimer, enableDefaultTransitionIndicator } from 'shared/ReactFeatureFlags';
import { clearEntangledAsyncTransitionTypes } from './ReactFiberTransitionTypes';
import noop from 'shared/noop';
import reportGlobalError from 'shared/reportGlobalError'; // If there are multiple, concurrent async actions, they are entangled. All
...
var currentEntangledActionThenable = null; // Track the default indicator for every root. undefined means we haven't
...
var isomorphicDefaultTransitionIndicator = undefined; // The clean up function for the currently running indicator.

var pendingIsomorphicIndicator = null; // The number of roots that have pending Transitions that depend on the
...
export function entangleAsyncAction(transition, thenable) {
  ...
  if (currentEntangledListeners === null) {
    ...
    if (enableDefaultTransitionIndicator) {
      needsIsomorphicIndicator = true; // We'll check if we need a default indicator in a microtask. Ensure
...
export function chainThenableValue(thenable, result) {
...
export function registerDefaultIndicator(onDefaultTransitionIndicator) {
  if (!enableDefaultTransitionIndicator) {
    return;
  }
  ...
  if (isomorphicDefaultTransitionIndicator === undefined) {
    ...
    isomorphicDefaultTransitionIndicator = null; // Stop any on-going indicator since it's now ambiguous.
