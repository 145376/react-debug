...
import {
  ...
  callComponentInDEV,
  callRenderInDEV,
} from "./ReactFiberCallUserSpace"; // A special exception that's used to unwind the stack when an update flows
// into a dehydrated boundary.

export var SelectiveHydrationException = new Error(
...
function updateOffscreenComponent(
  ...
  if (
    ...
    if (
      ...
      workInProgress.memoizedState = nextState; // push the cache pool even though we're going to bail out
    ...
    if (prevState !== null) {
      ...
      var _prevCachePool = null; // If the render that spawned this one accessed the cache pool, resume
      // using the same cache. Unless the parent changed, since that means
      // there was a refresh.
...
function updateLegacyHiddenComponent(current, workInProgress, renderLanes) {
  var nextProps = workInProgress.pendingProps; // Note: These happen to have identical begin phases, for now. We shouldn't hold
...
function mountActivityChildren(workInProgress, nextProps, renderLanes) {
  if (__DEV__) {
  ...
  var nextChildren = nextProps.children;
  var nextMode = nextProps.mode;
  var mode = workInProgress.mode;
  var offscreenChildProps = {
    mode: nextMode,
...
function retryActivityComponentWithoutHydrating(
  ...
  primaryChildFragment.flags |= Placement; // If we're not going to hydrate we can't leave it dehydrated if something
...
function updateActivityComponent(current, workInProgress, renderLanes) {
  var nextProps = workInProgress.pendingProps; // Check if the first pass suspended.
  ...
  if (current === null) {
    ...
    if (getIsHydrating()) {
      if (nextProps.mode === "hidden") {
        ...
        pushDehydratedActivitySuspenseHandler(workInProgress);
        var dehydrated = claimNextHydratableActivityInstance(workInProgress);
...
function updateTracingMarkerComponent(current, workInProgress, renderLanes) {
  ...
  var nextProps = workInProgress.pendingProps; // TODO: (luna) Only update the tracing marker if it's newly rendered or it's name changed.
...
function updateProfiler(current, workInProgress, renderLanes) {
  if (enableProfilerTimer) {
    ...
    if (enableProfilerCommitHooks) {
      ...
      workInProgress.flags |= Passive; // Reset effect durations for the next eventual effect phase.
...
function mountLazyComponent(
  ...
  var loggedComponent = getComponentNameFromType(Component) || Component; // This message intentionally doesn't mention ForwardRef or MemoComponent
...
function updateSuspenseOffscreenState(prevOffscreenState, renderLanes) {
  ...
  if (prevCachePool !== null) {
    var parentCache = isPrimaryRenderer
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  var didPrimaryChildrenDefer = (workInProgress.flags & DidDefer) !== NoFlags;
  ...
  if (current === null) {
    ...
    if (getIsHydrating()) {
      ...
      var dehydrated = claimNextHydratableSuspenseInstance(workInProgress);
...
function updateDehydratedSuspenseComponent(
  ...
  if (!didSuspend) {
    ...
    if (didReceiveUpdate || hasContextChanged) {
      ...
      workInProgress.flags |= DidCapture | Callback; // Leave the child in place. I.e. the dehydrated fragment.
...
function updateViewTransition(current, workInProgress, renderLanes) {
  ...
  var nextChildren = pendingProps.children;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function attemptEarlyBailoutIfNoScheduledUpdate(
  ...
  switch (workInProgress.tag) {
    ...
    case SuspenseComponent: {
      var _state = workInProgress.memoizedState;

      if (_state !== null) {
        if (_state.dehydrated !== null) {
...
function beginWork(current, workInProgress, renderLanes) {
  ...
  if (current !== null) {
    ...
    if (
      oldProps !== newProps ||
      hasLegacyContextChanged() || // Force a re-render if the implementation changed due to hot reload:
  ...
  switch (workInProgress.tag) {
    ...
    case MemoComponent: {
      ...
      _resolvedProps6 = disableDefaultPropsExceptForClasses
        ? _resolvedProps6
        : resolveDefaultPropsOnNonClassComponent(_type.type, _resolvedProps6);
      return updateMemoComponent(
        current,
        workInProgress,
        _type,
