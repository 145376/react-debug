...
import { disableLegacyContext, enableSchedulingProfiler, disableDefaultPropsExceptForClasses } from 'shared/ReactFeatureFlags';
...
import { enqueueUpdate, entangleTransitions, processUpdateQueue, checkHasForceUpdateAfterProcessing, resetHasForceUpdateBeforeProcessing, createUpdate, ReplaceState, ForceUpdate, initializeUpdateQueue, cloneUpdateQueue, suspendIfUpdateReadFromEntangledAsyncAction } from './ReactFiberClassUpdateQueue';
...
function checkClassInstance(workInProgress, ctor, newProps) {
  ...
  if (__DEV__) {
    ...
    if (disableLegacyContext) {
      ...
      if (ctor.childContextTypes && !didWarnAboutChildContextTypes.has(ctor)) {
        didWarnAboutChildContextTypes.add(ctor);
        console.error('%s uses the legacy childContextTypes API which will soon be removed. ' + 'Use React.createContext() instead. (https://react.dev/link/legacy-context)', name);
      }

      if (ctor.contextTypes && !didWarnAboutContextTypes.has(ctor)) {
        didWarnAboutContextTypes.add(ctor);
        console.error('%s uses the legacy contextTypes API which will soon be removed. ' + 'Use React.createContext() with static contextType instead. ' + '(https://react.dev/link/legacy-context)', name);
      }
    }
...
export function resolveClassComponentProps(Component, baseProps, // Only resolve default props if this is a lazy component. Otherwise, they
...
alreadyResolvedDefaultProps) {
  var newProps = baseProps; // Remove ref from the props object, if it exists.
