...
var SuspendedOnAction = 9; // When this is true, the work-in-progress fiber just suspended (or errored) and
// we've yet to unwind the stack. In some cases, we may yield to the main thread
// after this happens. If the fiber is pinged before we resume, we can retry
// immediately instead of unwinding the stack.

var workInProgressSuspendedReason = NotSuspended;
var workInProgressThrownValue = null; // Tracks whether any siblings were skipped during the unwind phase after
// something suspends. Used to determine whether to schedule another render
// to prewarm the skipped siblings.

var workInProgressRootDidSkipSuspendedSiblings = false; // Whether the work-in-progress render is the result of a prewarm/prerender.
// This tells us whether or not we should render the siblings after
// something suspends.

var workInProgressRootIsPrerendering = false; // Whether a ping listener was attached during this render. This is slightly
...
export var entangledRenderLanes = NoLanes; // Whether to root completed, errored, suspended, etc.
...
var workInProgressRootPingedLanes = NoLanes; // If this render scheduled deferred work, this is the lane of the deferred task.

var workInProgressDeferredLane = NoLane; // Represents the retry lanes that were spawned by this render and have not
// been pinged since, implying that they are still suspended.

var workInProgressSuspendedRetryLanes = NoLanes; // Errors that are thrown during the render phase.
...
var pendingEffectsRenderEndTime = -0; // Profiling-only
...
var pendingSuspendedCommitReason = IMMEDIATE_COMMIT; // Profiling-only
...
export function scheduleViewTransitionEvent(fiber, callback) {
  if (enableViewTransition) {
    if (callback != null) {
      var state = fiber.stateNode;
      var _instance = state.ref;

      if (_instance === null) {
        _instance = state.ref = createViewTransitionInstance(
      ...
      pendingViewTransitionEvents.push(callback.bind(null, _instance));
    }
  }
}
...
export function scheduleUpdateOnFiber(root, fiber, lane) {
  ...
  if (
    ...
    (root === workInProgressRoot &&
      (workInProgressSuspendedReason === SuspendedOnData ||
        workInProgressSuspendedReason === SuspendedOnAction)) || // Suspended commit phase
  ...
  if (
    ...
    if (root === workInProgressRoot) {
      ...
      if (workInProgressRootExitStatus === RootSuspendedWithDelay) {
        ...
        var _didAttemptEntireTree = false;
        markRootSuspended(
          root,
          workInProgressRootRenderLanes,
          workInProgressDeferredLane,
          _didAttemptEntireTree
        );
      }
    }
...
export function performWorkOnRoot(root, lanes, forceSync) {
  ...
  if (enableProfilerTimer && enableComponentPerformanceTrack) {
    if (workInProgressRootRenderLanes !== NoLanes && workInProgress !== null) {
      var yieldedFiber = workInProgress; // We've returned from yielding to the event loop. Let's log the time it took.

      var yieldEndTime = now();
  ...
  var shouldTimeSlice =
    (!forceSync &&
      ...
      !includesExpiredLane(root, lanes)) || // If we're prerendering, then we should use the concurrent work loop
  ...
  do {
    if (exitStatus === RootInProgress) {
      ...
      if (
        ...
        exitStatus = renderRootSync(root, lanes, false); // We assume the tree is now consistent because we didn't yield to any
        // concurrent events.
      ...
      if (exitStatus === RootFatalErrored) {
        ...
        prepareFreshStack(root, NoLanes); // Since this is a fatal error, we're going to pretend we attempted
        // the entire tree, to avoid scheduling a prerender.

        var _didAttemptEntireTree2 = true;
        markRootSuspended(root, lanes, NoLane, _didAttemptEntireTree2);
        break;
...
function recoverFromConcurrentError(
  ...
  var exitStatus = renderRootSync(root, errorRetryLanes, false);
...
function finishConcurrentRender(
  ...
  switch (exitStatus) {
    ...
    case RootSuspendedAtTheShell: {
      ...
      var didAttemptEntireTree = !workInProgressRootDidSkipSuspendedSiblings;
      markRootSuspended(
        root,
        lanes,
        workInProgressDeferredLane,
    ...
    if (
      ...
      if (msUntilTimeout > 10) {
        ...
        markRootSuspended(
          ...
          lanes,
          workInProgressDeferredLane,
          _didAttemptEntireTree3
        );
        var nextLanes = getNextLanes(root, NoLanes, true);
...
function commitRootWhenReady(
  ...
  root.timeoutHandle = noTimeout; // TODO: Combine retry throttling with Suspensey commits. Right now they run
  // one after the other.
  ...
  var isGestureTransition = enableGestureTransition && isGestureRender(lanes);
  ...
  if (isViewTransitionEligible || maySuspendCommit || isGestureTransition) {
    ...
    if (schedulePendingCommit !== null) {
      ...
      var didAttemptEntireTree = !didSkipSuspendedSiblings;
...
function prepareFreshStack(root, lanes) {
  if (enableProfilerTimer && enableComponentPerformanceTrack) {
    ...
    // recording starts.
    markAllLanesInOrder();
    var previousRenderStartTime = renderStartTime; // Starting a new render. Log the end of any previous renders and the
    // blocked time before the render started.

    recordRenderTime(); // If this was a restart, e.g. due to an interrupting update, then there's no space
    ...
    if (includesSyncLane(lanes) || includesBlockingLane(lanes)) {
      ...
      if (blockingSuspendedTime >= 0) {
        setCurrentTrackFromLanes(lanes);
        logSuspendedWithDelayPhase(
          blockingSuspendedTime, // Clamp the suspended time to the first event/update.
          clampedEventTime >= 0
            ? clampedEventTime
            : clampedUpdateTime >= 0
            ? clampedUpdateTime
    ...
    if (includesTransitionLane(lanes)) {
      ...
      if (transitionSuspendedTime >= 0) {
        setCurrentTrackFromLanes(lanes);
        logSuspendedWithDelayPhase(
          transitionSuspendedTime, // Clamp the suspended time to the first event/update.
          _clampedEventTime >= 0
            ? _clampedEventTime
            : _clampedUpdateTime >= 0
            ? _clampedUpdateTime
            : renderStartTime,
          lanes
        );
...
function handleThrow(root, thrownValue) {
  ...
  if (
    ...
    thrownValue = getSuspendedThenable(); // TODO: Suspending the work loop during the render phase is
...
export function renderDidSuspendDelayIfPossible() {
  ...
  if (
    !workInProgressRootDidSkipSuspendedSiblings && // Check if the root will be blocked from committing.
      ...
      if (
        ...
        switch (workInProgressSuspendedReason) {
          ...
          default: {
            ...
            var _reason = workInProgressSuspendedReason;
            workInProgressSuspendedReason = NotSuspended;
            workInProgressThrownValue = null;
            throwAndUnwindWorkLoop(root, unitOfWork, thrownValue, _reason);
            break;
          }
        }
      }
...
function workLoopConcurrent(nonIdle) {
  ...
  if (workInProgress !== null) {
    var yieldAfter = now() + (nonIdle ? 25 : 5);
...
function throwAndUnwindWorkLoop(
  ...
  if (unitOfWork.flags & Incomplete) {
    ...
    if (
      ...
      skipSiblings = true; // We intentionally don't set workInProgressRootDidSkipSuspendedSiblings,
      ...
      skipSiblings = true;
      workInProgressRootDidSkipSuspendedSiblings = true; // Because we're skipping the siblings, schedule an immediate retry of
      // this boundary.
      //
      ...
      if (
        ...
        var boundary = getSuspenseHandler();
...
function completeUnitOfWork(unitOfWork) {
  ...
  do {
    if ((completedWork.flags & Incomplete) !== NoFlags) {
      ...
      var skipSiblings = workInProgressRootDidSkipSuspendedSiblings;
...
function unwindUnitOfWork(unitOfWork, skipSiblings) {
  ...
  do {
    ...
    var next = unwindWork(current, incompleteWork, entangledRenderLanes); // Because this fiber did not complete, don't reset its lanes.
    ...
    if (enableProfilerTimer && (incompleteWork.mode & ProfileMode) !== NoMode) {
      ...
      stopProfilerTimerIfRunningAndRecordIncompleteDuration(incompleteWork); // Include the time spent working on failed children before continuing.
...
function commitRoot(
  ...
  if (
    ...
    if (enableYieldingBeforePassive) {
      ...
      scheduleCallback(NormalSchedulerPriority, function () {
        ...
        flushPassiveEffects(true); // This render triggered passive effects: release the root cache pool
  ...
  resetShouldStartViewTransition(); // The commit phase is broken into several sub-phases. We do a separate pass
  ...
  var subtreeHasBeforeMutationEffects =
    (finishedWork.subtreeFlags & (BeforeMutationMask | MutationMask)) !==
    NoFlags;
  var rootHasBeforeMutationEffect =
  ...
  var willStartViewTransition = shouldStartViewTransition;

  if (enableGestureTransition) {
    // Stop any gestures that were completed and is now being committed.
    if (root.stoppingGestures !== null) {
      stopCompletedGestures(root); // If we are in the process of stopping some gesture we shouldn't start
  ...
  if (enableViewTransition && willStartViewTransition) {
    ...
    // Flush synchronously.
    flushMutationEffects();
    flushLayoutEffects(); // Skip flushAfterMutationEffects
...
function flushMutationEffects() {
  ...
  var finishedWork = pendingFinishedWork;
  var lanes = pendingEffectsLanes;
  var subtreeMutationHasEffects =
    (finishedWork.subtreeFlags & MutationMask) !== NoFlags;
  var rootMutationHasEffect = (finishedWork.flags & MutationMask) !== NoFlags;
...
function flushLayoutEffects() {
  ...
  if (enableDefaultTransitionIndicator) {
    var cleanUpIndicator = root.pendingIndicator;
  ...
  var subtreeHasLayoutEffects =
    (finishedWork.subtreeFlags & LayoutMask) !== NoFlags;
  var rootHasLayoutEffect = (finishedWork.flags & LayoutMask) !== NoFlags;

  if (subtreeHasLayoutEffects || rootHasLayoutEffect) {
    var _prevTransition = ReactSharedInternals.T;
    ReactSharedInternals.T = null;
    ...
    try {
      ...
      ReactSharedInternals.T = _prevTransition;
    }
  }
...
function flushSpawnedWork() {
  if (
    pendingEffectsStatus !== PENDING_SPAWNED_WORK && // If a startViewTransition times out, we might flush this earlier than
  ...
  var finishedWork = pendingFinishedWork;
  var lanes = pendingEffectsLanes;
  var completedRenderEndTime = pendingEffectsRenderEndTime;
  var recoverableErrors = pendingRecoverableErrors;
  var didIncludeRenderPhaseUpdate = pendingDidIncludeRenderPhaseUpdate;
  var suspendedCommitReason = pendingSuspendedCommitReason;
  ...
  var passiveSubtreeMask =
    enableViewTransition && includesOnlyViewTransitionEligibleLanes(lanes)
      ? PassiveTransitionMask
      : PassiveMask;
  var rootDidHavePassiveEffects = // If this subtree rendered with profiling this commit, we need to visit it to log it.
    (enableProfilerTimer &&
      enableComponentPerformanceTrack &&
      finishedWork.actualDuration !== 0) ||
    (finishedWork.subtreeFlags & passiveSubtreeMask) !== NoFlags ||
  ...
  var remainingLanes = root.pendingLanes; // Check if there's remaining work on this root
  ...
  var renderPriority = lanesToEventPriority(lanes);
  ...
  if (enableViewTransition) {
    ...
    var pendingEvents = pendingViewTransitionEvents;
    var pendingTypes = pendingTransitionTypes;
    pendingTransitionTypes = null;
    ...
    if (pendingEvents !== null) {
      ...
      for (var _i = 0; _i < pendingEvents.length; _i++) {
        var viewTransitionEvent = pendingEvents[_i];
  ...
  ensureRootIsScheduled(root); // Read this again, since a passive effect might have updated it
...
var didWarnAboutInterruptedViewTransitions = false;
export function flushPendingEffects(wasDelayedCommit) {
  ...
  flushGestureAnimations();
  flushMutationEffects();
  flushLayoutEffects(); // Skip flushAfterMutation if we're forcing this early.
...
function flushPassiveEffects(wasDelayedCommit) {
  ...
  var root = pendingEffectsRoot; // Cache and clear the remaining lanes flag; it must be reset since this
...
export function captureCommitPhaseError(
  ...
  while (fiber !== null) {
    if (fiber.tag === HostRoot) {
      ...
      var ctor = fiber.type;
      var _instance2 = fiber.stateNode;

      if (
        typeof ctor.getDerivedStateFromError === "function" ||
        (typeof _instance2.componentDidCatch === "function" &&
          !isAlreadyFailedLegacyErrorBoundary(_instance2))
      ) {
        var errorInfo = createCapturedValueAtFiber(error, sourceFiber);
...
export function resolveRetryWakeable(boundaryFiber, wakeable) {
  ...
  switch (boundaryFiber.tag) {
    ...
    case OffscreenComponent: {
      var _instance3 = boundaryFiber.stateNode;
      retryCache = _instance3._retryCache;
      break;
    }
