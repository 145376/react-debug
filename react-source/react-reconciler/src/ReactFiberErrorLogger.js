...
import { bindToConsole } from './ReactFiberConfig'; // Side-channel since I'm not sure we want to make this part of the public API

var componentName = null;
var errorBoundaryName = null;
export function defaultOnUncaughtError(error, errorInfo) {
  ...
  if (__DEV__) {
    ...
    try {
      console.warn('%s\n\n%s\n', componentNameMessage, errorBoundaryMessage // We let our console.error wrapper add the component stack to the end.
      );
    } finally {// ignore
    }
  }
}
export function defaultOnCaughtError(error, errorInfo) {
  // Overriding this can silence these warnings e.g. for tests.
  // See https://github.com/facebook/react/pull/13384
  ...
  if (__DEV__) {
    ...
    try {
      if (typeof error === 'object' && error !== null && typeof error.environmentName === 'string') {
        ...
        bindToConsole('error', ['%o\n\n%s\n\n%s\n', error, componentNameMessage, recreateMessage // We let DevTools or console.createTask add the component stack to the end.
        ], error.environmentName)();
      } else {
        console.error('%o\n\n%s\n\n%s\n', error, componentNameMessage, recreateMessage // We let our DevTools or console.createTask add the component stack to the end.
        );
      }
    } finally {// ignore
    }
...
export function defaultOnRecoverableError(error, errorInfo) {
  reportGlobalError(error);
}
export function logUncaughtError(root, errorInfo) {
  try {
    if (__DEV__) {
...
export function logCaughtError(root, boundary, errorInfo) {
  try {
    if (__DEV__) {
      componentName = errorInfo.source ? getComponentNameFromFiber(errorInfo.source) : null;
