 ...
 * 
 */
import { includesLoadingIndicatorLanes, includesOnlySuspenseyCommitEligibleLanes, includesOnlyViewTransitionEligibleLanes } from './ReactFiberLane';
import { alwaysThrottleRetries, enableCreateEventHandleAPI, enableHiddenSubtreeInsertionEffectCleanup, enablePersistedModeClonedFlag, enableProfilerTimer, enableProfilerCommitHooks, enableSuspenseCallback, enableScopeAPI, enableUpdaterTracking, enableTransitionTracing, enableUseEffectEventHook, enableLegacyHidden, disableLegacyMode, enableComponentPerformanceTrack, enableViewTransition, enableFragmentRefs, enableEagerAlternateStateNodeCleanup, enableDefaultTransitionIndicator } from 'shared/ReactFeatureFlags';
import { FunctionComponent, ForwardRef, ClassComponent, HostRoot, HostComponent, HostHoistable, HostSingleton, HostText, HostPortal, Profiler, ActivityComponent, SuspenseComponent, DehydratedFragment, IncompleteClassComponent, MemoComponent, SimpleMemoComponent, SuspenseListComponent, ScopeComponent, OffscreenComponent, LegacyHiddenComponent, CacheComponent, TracingMarkerComponent, ViewTransitionComponent, Fragment } from './ReactWorkTags';
import { NoFlags, ContentReset, Placement, ChildDeletion, Snapshot, Update, Hydrate, Callback, Ref, Hydrating, Passive, BeforeMutationMask, BeforeAndAfterMutationTransitionMask, MutationMask, LayoutMask, PassiveMask, PassiveTransitionMask, Visibility, ShouldSuspendCommit, MaySuspendCommit, FormReset, Cloned, PerformedWork, ForceClientRender, DidCapture, AffectedParentLayout, ViewTransitionNamedStatic } from './ReactFiberFlags';
import { commitStartTime, pushNestedEffectDurations, popNestedEffectDurations, bubbleNestedEffectDurations, resetComponentEffectTimers, pushComponentEffectStart, popComponentEffectStart, pushComponentEffectDuration, popComponentEffectDuration, pushComponentEffectErrors, popComponentEffectErrors, componentEffectStartTime, componentEffectEndTime, componentEffectDuration, componentEffectErrors } from './ReactProfilerTimer';
import { logComponentRender, logComponentErrored, logComponentEffect, logComponentMount, logComponentUnmount, logComponentReappeared, logComponentDisappeared } from './ReactFiberPerformanceTrack';
import { ConcurrentMode, NoMode, ProfileMode } from './ReactTypeOfMode';
import { deferHiddenCallbacks } from './ReactFiberClassUpdateQueue';
import { supportsMutation, supportsPersistence, supportsHydration, supportsResources, supportsSingletons, clearSuspenseBoundary, clearSuspenseBoundaryFromContainer, createContainerChildSet, clearContainer, prepareScopeUpdate, prepareForCommit, beforeActiveInstanceBlur, detachDeletedInstance, getHoistableRoot, acquireResource, releaseResource, hydrateHoistable, mountHoistable, unmountHoistable, prepareToCommitHoistables, maySuspendCommitInSyncRender, suspendInstance, suspendResource, resetFormInstance, registerSuspenseInstanceRetry, cancelViewTransitionName, cancelRootViewTransitionName, restoreRootViewTransitionName, isSingletonScope, updateFragmentInstanceFiber } from './ReactFiberConfig';
import { captureCommitPhaseError, resolveRetryWakeable, markCommitTimeOfFallback, restorePendingUpdaters, addTransitionStartCallbackToPendingTransition, addTransitionProgressCallbackToPendingTransition, addTransitionCompleteCallbackToPendingTransition, addMarkerProgressCallbackToPendingTransition, addMarkerIncompleteCallbackToPendingTransition, addMarkerCompleteCallbackToPendingTransition, retryDehydratedSuspenseBoundary, scheduleViewTransitionEvent } from './ReactFiberWorkLoop';
import { HasEffect as HookHasEffect, Layout as HookLayout, Insertion as HookInsertion, Passive as HookPassive } from './ReactHookEffectTags';
...
import { commitHookLayoutEffects, commitHookLayoutUnmountEffects, commitHookEffectListMount, commitHookEffectListUnmount, commitHookPassiveMountEffects, commitHookPassiveUnmountEffects, commitClassLayoutLifecycles, commitClassDidMount, commitClassCallbacks, commitClassHiddenCallbacks, commitClassSnapshot, safelyCallComponentWillUnmount, safelyAttachRef, safelyDetachRef, commitProfilerUpdate, commitProfilerPostCommit, commitRootCallbacks } from './ReactFiberCommitEffects';
import { commitHostMount, commitHostHydratedInstance, commitHostUpdate, commitHostTextUpdate, commitHostResetTextContent, commitShowHideSuspenseBoundary, commitShowHideHostInstance, commitShowHideHostTextInstance, commitHostPlacement, commitHostRootContainerChildren, commitHostPortalContainerChildren, commitHostHydratedContainer, commitHostHydratedActivity, commitHostHydratedSuspense, commitHostRemoveChildFromContainer, commitHostRemoveChild, commitHostSingletonAcquisition, commitHostSingletonRelease, commitFragmentInstanceDeletionEffects, commitFragmentInstanceInsertionEffects } from './ReactFiberCommitHostEffects';
import { trackEnterViewTransitions, commitEnterViewTransitions, commitExitViewTransitions, commitBeforeUpdateViewTransition, commitNestedViewTransitions, restoreEnterOrExitViewTransitions, restoreUpdateViewTransition, restoreNestedViewTransitions, measureUpdateViewTransition, measureNestedViewTransitions, resetAppearingViewTransitions, trackAppearingViewTransition, viewTransitionCancelableChildren, pushViewTransitionCancelableScope, popViewTransitionCancelableScope } from './ReactFiberCommitViewTransitions';
import { viewTransitionMutationContext, pushRootMutationContext, pushMutationContext, popMutationContext, rootMutationContext } from './ReactFiberMutationTracking';
import { trackNamedViewTransition, untrackNamedViewTransition } from './ReactFiberDuplicateViewTransitions';
import { markIndicatorHandled } from './ReactFiberRootScheduler'; // Used during the commit phase to track the state of the Offscreen component stack.
...
export var shouldFireAfterActiveInstanceBlur = false; // Used during the commit phase to track whether a parent ViewTransition component
...
function isHydratingParent(current, finishedWork) {
  if (finishedWork.tag === ActivityComponent) {
    ...
    var _nextState = finishedWork.memoizedState;
    return _prevState !== null && _prevState.dehydrated !== null && (_nextState === null || _nextState.dehydrated === null);
  } else if (finishedWork.tag === HostRoot) {
    return current.memoizedState.isDehydrated && (finishedWork.flags & ForceClientRender) === NoFlags;
  } else {
    return false;
  }
}
...
export function commitBeforeMutationEffects(root, firstChild, committedLanes) {
  ...
  var isViewTransitionEligible = enableViewTransition && includesOnlyViewTransitionEligibleLanes(committedLanes);
  nextEffect = firstChild;
  commitBeforeMutationEffects_begin(isViewTransitionEligible); // We no longer need to track the active instance fiber

  focusedInstanceHandle = null; // We've found any matched pairs and can now reset.
...
function commitBeforeMutationEffects_begin(isViewTransitionEligible) {
  ...
  var subtreeMask = isViewTransitionEligible ? BeforeAndAfterMutationTransitionMask : BeforeMutationMask;
  ...
  while (nextEffect !== null) {
    ...
    if (enableViewTransition && fiber.tag === OffscreenComponent) {
      var isModernRoot = disableLegacyMode || (fiber.mode & ConcurrentMode) !== NoMode;
  ...
  switch (finishedWork.tag) {
    ...
    case ViewTransitionComponent:
      if (enableViewTransition) {
        if (isViewTransitionEligible) {
          if (current === null) {// This is a new mount. We should have handled this as part of the
...
function commitLayoutEffectOnFiber(finishedRoot, current, finishedWork, committedLanes) {
  ...
  var prevEffectErrors = pushComponentEffectErrors(); // When updating this function, also update reappearLayoutEffects, which does
    ...
    case Profiler:
      {
        ...
        if (flags & Update) {
          var _prevProfilerEffectDuration = pushNestedEffectDurations();

          recursivelyTraverseLayoutEffects(finishedRoot, finishedWork, committedLanes);
          var profilerInstance = finishedWork.stateNode;

          if (enableProfilerTimer && enableProfilerCommitHooks) {
            // Propagate layout effect durations to the next nearest Profiler ancestor.
            // Do not reset these values until the next render so DevTools has a chance to read them first.
            profilerInstance.effectDuration += bubbleNestedEffectDurations(_prevProfilerEffectDuration);
          }
    ...
    case SuspenseComponent:
      {
        ...
        if (flags & Callback) {
          ...
          // certainty.
          var finishedState = finishedWork.memoizedState;

          if (finishedState !== null) {
            var dehydrated = finishedState.dehydrated;

            if (dehydrated !== null) {
              // Register a callback to retry this boundary once the server has sent the result.
              var retry = retryDehydratedSuspenseBoundary.bind(null, finishedWork);
  ...
  if (enableProfilerTimer && enableProfilerCommitHooks && enableComponentPerformanceTrack && (finishedWork.mode & ProfileMode) !== NoMode && componentEffectStartTime >= 0 && componentEffectEndTime >= 0) {
    if (componentEffectDuration > 0.05) {
      logComponentEffect(finishedWork, componentEffectStartTime, componentEffectEndTime, componentEffectDuration, componentEffectErrors);
    }
    ...
    finishedWork.alternate === null && finishedWork.return !== null && finishedWork.return.alternate !== null && componentEffectEndTime - componentEffectStartTime > 0.05) {
      var isHydration = isHydratingParent(finishedWork.return.alternate, finishedWork.return);
...
function hideOrUnhideAllChildren(finishedWork, isHidden) {
  ...
  if (supportsMutation) {
    ...
    while (true) {
      if (node.tag === HostComponent || (supportsResources ? node.tag === HostHoistable : false)) {
...
function commitDeletionEffectsOnFiber(finishedRoot, nearestMountedAncestor, deletedFiber) {
  ...
  var prevEffectStart = pushComponentEffectStart();
  var prevEffectDuration = pushComponentEffectDuration();
  var prevEffectErrors = pushComponentEffectErrors(); // The cases in this outer switch modify the stack before they traverse
  ...
  if (enableProfilerTimer && enableProfilerCommitHooks && enableComponentPerformanceTrack && (deletedFiber.mode & ProfileMode) !== NoMode && componentEffectStartTime >= 0 && componentEffectEndTime >= 0 && componentEffectDuration > 0.05) {
    logComponentEffect(deletedFiber, componentEffectStartTime, componentEffectEndTime, componentEffectDuration, componentEffectErrors);
  }
...
function commitMutationEffectsOnFiber(finishedWork, root, lanes) {
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent:
      {
        ...
        if (flags & Update) {
          commitHookEffectListUnmount(HookInsertion | HookHasEffect, finishedWork, finishedWork.return); // TODO: Use a commitHookInsertionUnmountEffects wrapper to record timings.
        ...
        if (supportsMutation) {
          ...
          if (flags & Update) {
            ...
            if (instance != null) {
              ...
              commitHostUpdate(finishedWork, _newProps, _oldProps);
            }
          }
    ...
    case Profiler:
      {
        var _prevProfilerEffectDuration2 = pushNestedEffectDurations();

        recursivelyTraverseMutationEffects(root, finishedWork, lanes);
        commitReconciliationEffects(finishedWork, lanes);

        if (enableProfilerTimer && enableProfilerCommitHooks) {
          var profilerInstance = finishedWork.stateNode; // Propagate layout effect durations to the next nearest Profiler ancestor.
          // Do not reset these values until the next render so DevTools has a chance to read them first.

          profilerInstance.effectDuration += bubbleNestedEffectDurations(_prevProfilerEffectDuration2);
        }
    ...
    case SuspenseComponent:
      {
        recursivelyTraverseMutationEffects(root, finishedWork, lanes);
        commitReconciliationEffects(finishedWork, lanes); // TODO: We should mark a flag on the Suspense fiber itself, rather than
        ...
        if (flags & Update) {
          ...
          var _retryQueue = finishedWork.updateQueue;
    ...
    case OffscreenComponent:
      {
        ...
        if (disableLegacyMode || finishedWork.mode & ConcurrentMode) {
          ...
          if ( // If this was the root of the reappear.
        ...
        if (flags & Update) {
          ...
          if (offscreenQueue !== null) {
            var _retryQueue2 = offscreenQueue.retryQueue;
    ...
    case SuspenseListComponent:
      {
        ...
        if (flags & Update) {
          var _retryQueue3 = finishedWork.updateQueue;

          if (_retryQueue3 !== null) {
            finishedWork.updateQueue = null;
            attachSuspenseRetryListeners(finishedWork, _retryQueue3);
          }
        }
    ...
    case ViewTransitionComponent:
      {
        if (enableViewTransition) {
          ...
          var _prevMutationContext = pushMutationContext();

          var prevUpdate = inUpdateViewTransition;
          var isViewTransitionEligible = enableViewTransition && includesOnlyViewTransitionEligibleLanes(lanes);
          ...
          popMutationContext(_prevMutationContext);
          break;
        }
    ...
    case ScopeComponent:
      {
        if (enableScopeAPI) {
          recursivelyTraverseMutationEffects(root, finishedWork, lanes);
          commitReconciliationEffects(finishedWork, lanes); // TODO: This is a temporary solution that allowed us to transition away
...
function commitReconciliationEffects(finishedWork, committedLanes) {
  ...
  if (flags & Placement) {
    commitHostPlacement(finishedWork); // Clear the "placement" from effect tag so that we know that this is
...
export function commitAfterMutationEffects(root, finishedWork, committedLanes) {
  if (!enableViewTransition) {
...
function recursivelyTraverseAfterMutationEffects(root, parentFiber, lanes) {
...
function commitAfterMutationEffectsOnFiber(finishedWork, root, lanes) {
  var current = finishedWork.alternate;
    ...
    case OffscreenComponent:
      {
        ...
        if (isModernRoot) {
          ...
          if (isHidden) {// The Offscreen tree is hidden. Skip over its after mutation effects.
          } else {
            // The Offscreen tree is visible.
            var wasHidden = current.memoizedState !== null;

            if (wasHidden) {
              commitEnterViewTransitions(finishedWork, false); // If it was previous hidden then the children are treated as enter
    ...
    case ViewTransitionComponent:
      {
        ...
        var inViewport = measureUpdateViewTransition(current, finishedWork, false);
        ...
        if ((finishedWork.flags & Update) === NoFlags || !inViewport) {
          ...
          var props = finishedWork.memoizedProps;
          scheduleViewTransitionEvent(finishedWork, props.onUpdate); // If this boundary did update, we cannot cancel its children so those are dropped.
        ...
        if ((finishedWork.flags & AffectedParentLayout) !== NoFlags) {
          ...
          viewTransitionContextChanged = _prevContextChanged;
        }
...
includeWorkInProgressEffects) {
  var prevEffectStart = pushComponentEffectStart();
  var prevEffectDuration = pushComponentEffectDuration();
  var prevEffectErrors = pushComponentEffectErrors(); // Turn on layout effects in a tree that previously disappeared.
    ...
    case ClassComponent:
      {
        ...
        commitClassHiddenCallbacks(finishedWork); // If this is newly finished work, check for setState callbacks
    ...
    case HostSingleton:
      {
        if (supportsSingletons) {
          ...
          // is it seemingly requires an extra traversal because we need to move the
          // disappear effect into a phase before the appear phase
          commitHostSingletonAcquisition(finishedWork); // We fall through to the HostComponent case below.
        } // Fallthrough
...
export function commitPassiveMountEffects(root, finishedWork, committedLanes, committedTransitions, renderEndTime // Profiling-only
) {
  resetComponentEffectTimers();
  commitPassiveMountOnFiber(root, finishedWork, committedLanes, committedTransitions, enableProfilerTimer && enableComponentPerformanceTrack ? renderEndTime : 0);
}

function recursivelyTraversePassiveMountEffects(root, parentFiber, committedLanes, committedTransitions, endTime // Profiling-only. The start time of the next Fiber or root completion.
) {
  var isViewTransitionEligible = enableViewTransition && includesOnlyViewTransitionEligibleLanes(committedLanes); // TODO: We could optimize this by marking these with the Passive subtree flag in the render phase.

  var subtreeMask = isViewTransitionEligible ? PassiveTransitionMask : PassiveMask;
  ...
  enableProfilerTimer && enableComponentPerformanceTrack && parentFiber.actualDuration !== 0 && (parentFiber.alternate === null || parentFiber.alternate.child !== parentFiber.child)) {
    ...
    while (child !== null) {
      if (enableProfilerTimer && enableComponentPerformanceTrack) {
        ...
        commitPassiveMountOnFiber(root, child, committedLanes, committedTransitions, nextSibling !== null ? nextSibling.actualStartTime : endTime);
        child = nextSibling;
      } else {
...
function commitPassiveMountOnFiber(finishedRoot, finishedWork, committedLanes, committedTransitions, endTime // Profiling-only. The start time of the next Fiber or root completion.
) {
  var prevEffectStart = pushComponentEffectStart();
  var prevEffectDuration = pushComponentEffectDuration();
  var prevEffectErrors = pushComponentEffectErrors();
  var isViewTransitionEligible = enableViewTransition ? includesOnlyViewTransitionEligibleLanes(committedLanes) : false;
  ...
  switch (finishedWork.tag) {
    ...
    case ClassComponent:
      {
        ...
        if (enableProfilerTimer && enableComponentPerformanceTrack && (finishedWork.mode & ProfileMode) !== NoMode && finishedWork.actualStartTime > 0) {
          if ((finishedWork.flags & DidCapture) !== NoFlags) {
            logComponentErrored(finishedWork, finishedWork.actualStartTime, endTime, // TODO: The captured values are all hidden inside the updater/callback closures so
    ...
    case Profiler:
      {
        ...
        if (flags & Passive) {
          var _prevProfilerEffectDuration3 = pushNestedEffectDurations();
          ...
          if (enableProfilerTimer && enableProfilerCommitHooks) {
            ...
            profilerInstance.passiveEffectDuration += bubbleNestedEffectDurations(_prevProfilerEffectDuration3);
          }

          commitProfilerPostCommit(finishedWork, finishedWork.alternate, // This value will still reflect the previous commit phase.
          // It does not get reset until the start of the next commit phase.
          commitStartTime, profilerInstance.passiveEffectDuration);
        } else {
          recursivelyTraversePassiveMountEffects(finishedRoot, finishedWork, committedLanes, committedTransitions, endTime);
        }
    ...
    case ActivityComponent:
      {
        var _wasInHydratedSubtree = inHydratedSubtree;

        if (enableProfilerTimer && enableComponentPerformanceTrack) {
          var prevState = finishedWork.alternate !== null ? finishedWork.alternate.memoizedState : null;
          var nextState = finishedWork.memoizedState;

          if (prevState !== null && nextState === null) {
            // This was dehydrated but is no longer dehydrated. We may have now either hydrated it
            // or client rendered it.
            var deletions = finishedWork.deletions;
            ...
            if (deletions !== null && deletions.length > 0 && deletions[0].tag === DehydratedFragment) {
              ...
              var hydrationErrors = prevState.hydrationErrors; // If there were no hydration errors, that suggests that this was an intentional client
        ...
        if (enableProfilerTimer && enableComponentPerformanceTrack) {
          inHydratedSubtree = _wasInHydratedSubtree;
        }
    ...
    case SuspenseComponent:
      {
        var _wasInHydratedSubtree2 = inHydratedSubtree;

        if (enableProfilerTimer && enableComponentPerformanceTrack) {
          var _prevState2 = finishedWork.alternate !== null ? finishedWork.alternate.memoizedState : null;

          var _nextState2 = finishedWork.memoizedState;

          if (_prevState2 !== null && _prevState2.dehydrated !== null && (_nextState2 === null || _nextState2.dehydrated === null)) {
            // This was dehydrated but is no longer dehydrated. We may have now either hydrated it
            // or client rendered it.
            var _deletions = finishedWork.deletions;

            if (_deletions !== null && _deletions.length > 0 && _deletions[0].tag === DehydratedFragment) {
              // This was an abandoned hydration that deleted the dehydrated fragment. That means we
              // are not hydrating this Suspense boundary.
              inHydratedSubtree = false;
              var _hydrationErrors = _prevState2.hydrationErrors; // If there were no hydration errors, that suggests that this was an intentional client
              // rendered boundary. Such as postpone.

              if (_hydrationErrors !== null) {
                var _startTime = finishedWork.actualStartTime;
                logComponentErrored(finishedWork, _startTime, endTime, _hydrationErrors);
              }
            } else {
              // If any children committed they were hydrated.
              inHydratedSubtree = true;
        ...
        if (enableProfilerTimer && enableComponentPerformanceTrack) {
          inHydratedSubtree = _wasInHydratedSubtree2;
        }
    ...
    case OffscreenComponent:
      {
        ...
        var _nextState3 = finishedWork.memoizedState;
        var isHidden = _nextState3 !== null;

        if (isHidden) {
          if (isViewTransitionEligible && _current !== null && _current.memoizedState === null) {
          ...
          if (_instance._visibility & OffscreenPassiveEffectsConnected) {
          ...
          if (_instance._visibility & OffscreenPassiveEffectsConnected) {
            ...
            if (enableProfilerTimer && enableProfilerCommitHooks && enableComponentPerformanceTrack && (finishedWork.mode & ProfileMode) !== NoMode && !inHydratedSubtree) {
              ...
              var _startTime2 = finishedWork.actualStartTime;

              if (_startTime2 >= 0 && endTime - _startTime2 > 0.05) {
                logComponentReappeared(finishedWork, _startTime2, endTime);
              }
        ...
        if (flags & Passive) {
          commitOffscreenPassiveMountEffects(_current, finishedWork, _instance);
        }
    ...
    case ViewTransitionComponent:
      {
        if (enableViewTransition) {
          if (isViewTransitionEligible) {
            ...
            if (_current3 === null) {// This is a new mount. We should have handled this as part of the
              // Placement effect or it is deeper inside a entering transition.
            } else {
              // Something mutated within this subtree. This might have caused
              // something to cross-fade if we didn't already cancel it.
              // If not, restore it.
              restoreUpdateViewTransition(_current3, finishedWork);
            }
          }
  ...
  if (enableProfilerTimer && enableProfilerCommitHooks && enableComponentPerformanceTrack && (finishedWork.mode & ProfileMode) !== NoMode) {
    var isMount = !inHydratedSubtree && finishedWork.alternate === null && finishedWork.return !== null && finishedWork.return.alternate !== null;

    if (isMount) {
      // Log the mount in the render phase.
      var _startTime3 = finishedWork.actualStartTime;

      if (_startTime3 >= 0 && endTime - _startTime3 > 0.05) {
        logComponentMount(finishedWork, _startTime3, endTime);
      }
    }
...
function recursivelyTraverseReconnectPassiveEffects(finishedRoot, parentFiber, committedLanes, committedTransitions, includeWorkInProgressEffects, endTime) {
  ...
  while (child !== null) {
    if (enableProfilerTimer && enableComponentPerformanceTrack) {
      var nextSibling = child.sibling;
      reconnectPassiveEffects(finishedRoot, child, committedLanes, committedTransitions, childShouldIncludeWorkInProgressEffects, nextSibling !== null ? nextSibling.actualStartTime : endTime);
      child = nextSibling;
    } else {
      reconnectPassiveEffects(finishedRoot, child, committedLanes, committedTransitions, childShouldIncludeWorkInProgressEffects, endTime);
      child = child.sibling;
    }
  }
}
...
// from a previously committed tree. We cannot check non-static flags if the
// node was reused.
includeWorkInProgressEffects, endTime // Profiling-only. The start time of the next Fiber or root completion.
) {
  var prevEffectStart = pushComponentEffectStart();
  var prevEffectDuration = pushComponentEffectDuration();
  var prevEffectErrors = pushComponentEffectErrors(); // If this component rendered in Profiling mode (DEV or in Profiler component) then log its
  // render time. We do this after the fact in the passive effect to avoid the overhead of this
  // getting in the way of the render characteristics and avoid the overhead of unwinding
  // uncommitted renders.
  ...
  switch (finishedWork.tag) {
    ...
    case SimpleMemoComponent:
      {
        recursivelyTraverseReconnectPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions, includeWorkInProgressEffects, endTime); // TODO: Check for PassiveStatic flag
    ...
    case OffscreenComponent:
      {
        ...
        if (isHidden) {
          if (_instance2._visibility & OffscreenPassiveEffectsConnected) {
            // The effects are currently connected. Update them.
            recursivelyTraverseReconnectPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions, includeWorkInProgressEffects, endTime);
          } else {
            if (disableLegacyMode || finishedWork.mode & ConcurrentMode) {
        ...
        if (includeWorkInProgressEffects && flags & Passive) {
          ...
          commitOffscreenPassiveMountEffects(_current4, finishedWork, _instance2);
        }
...
function recursivelyTraverseAtomicPassiveEffects(finishedRoot, parentFiber, committedLanes, committedTransitions, endTime // Profiling-only. The start time of the next Fiber or root completion.
) {
  ...
  if (parentFiber.subtreeFlags & PassiveMask) {
    ...
    while (child !== null) {
      if (enableProfilerTimer && enableComponentPerformanceTrack) {
        var nextSibling = child.sibling;
        commitAtomicPassiveEffects(finishedRoot, child, committedLanes, committedTransitions, nextSibling !== null ? nextSibling.actualStartTime : endTime);
        child = nextSibling;
      } else {
...
function commitAtomicPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions, endTime // Profiling-only. The start time of the next Fiber or root completion.
) {
  // If this component rendered in Profiling mode (DEV or in Profiler component) then log its
    ...
    case CacheComponent:
      {
        ...
        if (flags & Passive) {
          // TODO: Pass `current` as argument to this function
          var _current6 = finishedWork.alternate;
          commitCachePassiveMountEffect(_current6, finishedWork);
        }
...
export function accumulateSuspenseyCommit(finishedWork, committedLanes) {
  resetAppearingViewTransitions();
    ...
    case HostHoistable:
      {
        ...
        if (fiber.flags & suspenseyCommitFlag) {
          if (fiber.memoizedState !== null) {
            ...
            var type = fiber.type;
            var props = fiber.memoizedProps; // TODO: Allow sync lanes to suspend too with an opt-in.
    ...
    case HostComponent:
      {
        ...
        if (fiber.flags & suspenseyCommitFlag) {
          var _instance3 = fiber.stateNode;
          var _type = fiber.type;
          var _props = fiber.memoizedProps; // TODO: Allow sync lanes to suspend too with an opt-in.

          if (includesOnlySuspenseyCommitEligibleLanes(committedLanes) || maySuspendCommitInSyncRender(_type, _props)) {
            suspendInstance(_instance3, _type, _props);
          }
        }
    ...
    case ViewTransitionComponent:
      {
        if (enableViewTransition) {
          if ((fiber.flags & suspenseyCommitFlag) !== NoFlags) {
            var _props2 = fiber.memoizedProps;
            ...
            if (name != null && name !== 'auto') {
              ...
              var state = fiber.stateNode; // Reset the pair in case we didn't end up restoring the instance in previous commits.
...
function recursivelyTraversePassiveUnmountEffects(parentFiber) {
  ...
  if ((parentFiber.flags & ChildDeletion) !== NoFlags) {
    if (deletions !== null) {
      for (var i = 0; i < deletions.length; i++) {
        var childToDelete = deletions[i];
        var prevEffectStart = pushComponentEffectStart(); // TODO: Convert this to use recursion
...
function commitPassiveUnmountOnFiber(finishedWork) {
  ...
  switch (finishedWork.tag) {
    ...
    case HostRoot:
      {
        ...
        if (enableProfilerTimer && enableProfilerCommitHooks) {
          var finishedRoot = finishedWork.stateNode;
          finishedRoot.passiveEffectDuration += popNestedEffectDurations(prevProfilerEffectDuration);
        }
    ...
    case Profiler:
      {
        var _prevProfilerEffectDuration4 = pushNestedEffectDurations();
        ...
        if (enableProfilerTimer && enableProfilerCommitHooks) {
          ...
          profilerInstance.passiveEffectDuration += bubbleNestedEffectDurations(_prevProfilerEffectDuration4);
        }
...
function commitPassiveUnmountInsideDeletedTreeOnFiber(current, nearestMountedAncestor) {
    ...
    case TracingMarkerComponent:
      {
        if (enableTransitionTracing) {
          // We need to mark this fiber's parents as deleted
          var _instance4 = current.stateNode;
          var _transitions = _instance4.transitions;
  ...
  if (enableProfilerTimer && enableProfilerCommitHooks && enableComponentPerformanceTrack && (current.mode & ProfileMode) !== NoMode && componentEffectStartTime >= 0 && componentEffectEndTime >= 0 && componentEffectDuration > 0.05) {
    logComponentEffect(current, componentEffectStartTime, componentEffectEndTime, componentEffectDuration, componentEffectErrors);
  }
